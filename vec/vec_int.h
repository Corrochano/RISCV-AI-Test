...
#if defined(CPU_CAPABILITY_RISCV)

#define VEC_INT_RISCV_TEMPLATE(vl, bit)                                         \
  template <>                                                                 \
  ...
  class Vectorized<int##bit##_t> {                                            \
   private:                                                                   \
    vls_int##bit##_t values;                                                  \
                                                                              \
   public:
    /* Necesitamos VL todo el rato */                                         \   
    inline size_t get_rvv_vl() {                                              \
    ...
    Vectorized<int##bit##_t> operator==(                                      \
      ...
      /*1.Obtener el VL (Vector Length) actual para el tipo de dato*/         \
      size_t vl = get_rvv_vl();                                               \
      /*2.Realizar la comparación: devuelve una máscara (vbool_t)*/           \
      /*Suponiendo que 'values' es tu vint##bit##m1_t*/                       \
      vbool##bit##_t mask = __riscv_vmseq_vv_i##bit##m1_b##bit(values, other.values, vl); \
      /*3.Seleccionar entre el vector de TRUE (todos 1s) y FALSE (todos 0s)/* \
      /*En RISC-V, esto se hace con vmerge:*/                                 \
      /*Si el bit en mask es 1, elige el primer valor (op_true); si es 0, el segundo (op_false)*/ \
      return __riscv_vmerge_vvm_i##bit##m1(                                   \
        ALL_S##bit##_FALSE_MASK, /*Valor si la máscara es 0*/                 \
        ALL_S##bit##_TRUE_MASK,  /*Valor si la máscara es 1*/                 \
        mask,                                                                 \
        vl                                                                    \
      );                                                                      \
    }                                                                         \
    Vectorized<int##bit##_t> operator!=(                                      \
        const Vectorized<int##bit##_t>& other) const {                        \
      size_t vl = get_rvv_vl();                                               \
      vbool##bit##_t mask = __riscv_vmsne_vv_i##bit##m1_b##bit(values, other.values, vl); \
      return __riscv_vmerge_vvm_i##bit##m1(                                   \
        ALL_S##bit##_FALSE_MASK,                                              \
        ALL_S##bit##_TRUE_MASK,                                               \
        mask,                                                                 \
        vl                                                                    \
      );                                                                      \
    }                                                                         \
    Vectorized<int##bit##_t> operator<(                                       \
        const Vectorized<int##bit##_t>& other) const {                        \
      size_t vl = get_rvv_vl();                                               \
      vbool##bit##_t mask = __riscv_vmslt_vv_i##bit##m1_b##bit(values, other.values, vl); \
      return __riscv_vmerge_vvm_i##bit##m1(                                   \
        ALL_S##bit##_FALSE_MASK,                                              \
        ALL_S##bit##_TRUE_MASK,                                               \
        mask,                                                                 \
    ...
    Vectorized<int##bit##_t> operator<=(                                      \
        const Vectorized<int##bit##_t>& other) const {                        \
      size_t vl = get_rvv_vl();                                               \
      vbool##bit##_t mask = __riscv_vmsle_vv_i##bit##m1_b##bit(values, other.values, vl); \
      return __riscv_vmerge_vvm_i##bit##m1(                                   \
        ALL_S##bit##_FALSE_MASK,                                              \
        ALL_S##bit##_TRUE_MASK,                                               \
        mask,                                                                 \
    ...
    Vectorized<int##bit##_t> operator>(                                       \
        const Vectorized<int##bit##_t>& other) const {                        \
      size_t vl = get_rvv_vl();                                               \
      vbool##bit##_t mask = __riscv_vmsgt_vv_i##bit##m1_b##bit(values, other.values, vl); \
      return __riscv_vmerge_vvm_i##bit##m1(                                   \
        ALL_S##bit##_FALSE_MASK,                                              \
        ALL_S##bit##_TRUE_MASK,                                               \
        mask,                                                                 \
    ...
    Vectorized<int##bit##_t> operator>=(                                      \
        const Vectorized<int##bit##_t>& other) const {                        \
      size_t vl = get_rvv_vl();                                               \
      vbool##bit##_t mask = __riscv_vmsge_vv_i##bit##m1_b##bit(values, other.values, vl); \
      return __riscv_vmerge_vvm_i##bit##m1(                                   \
        ALL_S##bit##_FALSE_MASK,                                              \
        ALL_S##bit##_TRUE_MASK,                                               \
        mask,                                                                 \
  ...
  Vectorized<int##bit##_t> inline operator+(                                       \
      ...
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vadd_vv_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator+(                                       \
      const Vectorized<int##bit##_t>& a, int##bit##_t b) {                              \
      /* Version: Vector + Scalar */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vadd_vx_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator-(                                       \
      const Vectorized<int##bit##_t>& a, const Vectorized<int##bit##_t>& b) {           \
      /* Version: Vector + Vector */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vsub_vv_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator-(                                       \
      const Vectorized<int##bit##_t>& a, int##bit##_t b) {                              \
      /* Version: Vector + Scalar */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vsub_vx_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator*(                                       \
      const Vectorized<int##bit##_t>& a, const Vectorized<int##bit##_t>& b) {           \
      /* Version: Vector + Vector */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vmul_vv_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator*(                                       \
      const Vectorized<int##bit##_t>& a, int##bit##_t b) {                              \
      /* Version: Vector + Scalar */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vmul_vx_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator*(                                       \
      const Vectorized<int##bit##_t>& a, const Vectorized<int##bit##_t>& b) {           \
  ...
  Vectorized<int##bit##_t> inline maximum(                                         \
      ...
      /* Version: Vector + Vector */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vmax_vv_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline maximum(                                         \
      const Vectorized<int##bit##_t>& a, int##bit##_t b) {                              \
      /* Version: Vector + Scalar */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vmax_vx_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline minimum(                                         \
      ...
      /* Version: Vector + Vector */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vmin_vv_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline minimum(                                         \
      const Vectorized<int##bit##_t>& a, int##bit##_t b) {                              \
      /* Version: Vector + Scalar */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vmin_vx_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline clamp(           /*TODO Clamps*/            \
      const Vectorized<int##bit##_t>& a,                                      \
  ...
  Vectorized<int##bit##_t> inline operator&(                                       \
      ...
      /* Version: Vector + Scalar */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vand_vx_i##bit##m1(a, b, vl);                                   \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator&(                                       \
      const Vectorized<int##bit##_t>& a, int##bit##_t b) {                              \
  ...
  Vectorized<int##bit##_t> inline operator|(                                       \
      ...
      /* Version: Vector + Scalar */                                          \
      size_t vl = get_rvv_vl();                                               \
    return __riscv_vor_vx_i##bit##m1(a, b, vl);                                    \
  }                                                                           \
  template <>                                                                 \
  Vectorized<int##bit##_t> inline operator|(                                       \
      const Vectorized<int##bit##_t>& a, int##bit##_t b) {                              \
